...
func (o *GetItemsNameURL) Build() (*url.URL, error) {
	...
	var _path = "/items/{name}"

	name := o.Name
	if name != "" {
		_path = strings.Replace(_path, "{name}", name, -1)
	} else {
		return nil, errors.New("Name is required on GetItemsNameURL")
	}
...
func (o *GetItemsNameURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetItemsNameURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetItemsNameURL")
	}
